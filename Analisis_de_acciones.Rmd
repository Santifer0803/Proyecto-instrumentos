...
# Formato de fecha
datos.bitcoin$Fecha <-
  as.Date(datos.bitcoin$Fecha, format = "%d.%m.%Y")
...
datos.bitcoin <- datos.bitcoin %>%
  mutate(
    miles = as.numeric(gsub("K", "", datos.bitcoin$Vol)) * 1000,
...
datos.bitcoin$Vol <-
  ifelse(
    !is.na(datos.bitcoin$miles),
    datos.bitcoin$miles,
    ifelse(
      !is.na(datos.bitcoin$millones),
      datos.bitcoin$millones,
      ifelse(!is.na(datos.bitcoin$billones), datos.bitcoin$billones, NA)
    )
  )
...
ggplot(df.auxiliar, aes(x = fecha)) +
  geom_line(aes(y = diferencia),
            color = "orangered2" ,
            linewidth = 0.5) +
  labs(x = "Cambio etereum", y = "Cambio bitcoin", caption = "Fuente: Elaboración propia") +
  theme_minimal()
```
